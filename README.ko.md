...
- **사실상 공짜입니다.** 실사용 24시간 동안 idle wakeup 3회, `phys_footprint` 약 4MB, CPU 0.59초.
  폴링도 반복 타이머도 없이 `CFRunLoopRun()`에 파킹해 있다가 CoreAudio 알림이 올 때만 깨어납니다.
...
- **미해결 사건 2건, 그중 하나는 `micpeg status`로 드러나지 않습니다.** 2026-09-10에 에이전트가
  `PINNED`를 보고하는 동안 실제 기본 입력이 약 1.5시간 블루투스 헤드셋에 머물렀고 그동안 로그가
  0줄이었습니다. 2026-09-11에는 같은 형태가 42분간 재발했고, 이때는 데몬이 그 헤드셋을 정착된
  장치로 분류하고 물러나는 것으로 끝났습니다. 두 건 모두 데몬 자신이 그 헤드셋에서 다른 장치로
  쓴 직후에 일어났습니다. 잠자기, `coreaudiod` 재시작, 프로세스 재기동, 큐 교착, 리스너 사망은
  모두 배제했고, 알림 유실과 오래된 값에 대한 판정 두 가지가 남은 설명이며 재현 시도 5회는 모두
  실패했습니다. 조용히 포기하던 경로 3곳을 보험으로 보강했습니다. 첫 번째 형태는 `micpeg status`의
  `state:`와 `default input:`이 어긋나 보입니다. 두 번째는 `YIELDED`로 정상처럼 보이지만, 헤드셋을
  분리할 때까지 보호가 꺼져 있습니다. 둘 다 `micpeg on`으로 즉시 복구됩니다.
