...
Principles 1 and 6 are enforced structurally, not by discipline. CI greps for all three:
...
- Splitting the 1,228-line `main.swift` is still a separate, test-backed task. Extracting the
...
```
Sources/MicpegAudio/       # (planned) read-only CoreAudio helpers, shared. No writes.
